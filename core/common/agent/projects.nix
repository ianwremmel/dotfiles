{ config, pkgs, lib, ... }:
# Agent-host runtime bootstrap shared by every environment that opts into the
# agent bundle: the github.com bot-key wiring, credential restore, and project
# cloning. The only per-host difference is which repos to clone, exposed as
# `dotfiles.agent.reposFile`. Linux-gated — an agent host is a Linux box you SSH
# into; a macOS build of this bundle gets none of it.
let
...
in
{
  options.dotfiles.agent.reposFile = lib.mkOption {
    type = lib.types.nullOr lib.types.path;
    default = null;
    description = ''
      Path to a newline-separated list of `owner/name` GitHub slugs to clone (or
      fetch) into ~/projects on every apply. `#` starts a comment. Null (the
      default) clones nothing, which is how the public `agent-autonomous`
      environment ships — a private environment supplies its own list.
    '';
  };
  ...
  config = lib.mkIf pkgs.stdenv.isLinux {
    ...
    # Project repos: clone (or fetch) the slugs in reposFile into ~/projects.
    # Runs before writeHomelabMcp so ~/projects/homelab exists when that entry
    # writes the project's .mcp.json. Defined only when a reposFile is set.
    home.activation.cloneAgentProjects = lib.mkIf (cfg.reposFile != null) (
