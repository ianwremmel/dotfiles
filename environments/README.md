...
This directory (renamed from `nix/`) holds the Nix configuration that manages
the dotfiles via [home-manager](https://github.com/nix-community/home-manager)
(user-level) and [nix-darwin](https://github.com/nix-darwin/nix-darwin)
(macOS system-level). The core `flake.nix` here is a *library*; each
environment (`default`, `agent`) is its own [Nix
flake](https://nixos.wiki/wiki/Flakes) that consumes the core and emits the
actual configurations. `./apply` activates the selected environment
automatically.
...
To build/activate by hand after Nix is installed (run from the repo root):
    ...
    nix $flags build "path:environments/$env#homeConfigurations.\"$sys\".activationPackage" \
      --override-input public path:environments --out-link "$out"
    "$out/activate"

(Flakes are not enabled by default on a fresh Linux single-user install, hence
the `--extra-experimental-features` flag. `--override-input public
path:environments` points the env flake at the local core, which also makes the
untracked `environments/host.nix` visible — `./apply` generates that file; the
env flake throws without it. A private env builds the same way from
`custom_environments/<env>` — see Private environments.)
...
field in `host.nix`. The untracked `environments/host.nix` (generated by
...
1. **The `public` input is auto-redirected to your local source.** `lib/nix`
   builds the private flake with `--override-input public path:…/environments`,
...
- **Remove Nix entirely:** delete `lib/nix` and `environments/`, then uninstall Nix —
